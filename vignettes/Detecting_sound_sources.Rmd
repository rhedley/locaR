...
Before we get started, we should convert the example data from .mp3 to .wav. Many of the functions in `locaR` work exclusively with wav files.

```{r}
x <- list.files(system.file('data', package = 'locaR'), full.names = T, pattern='mp3$')

for(i in x) {
  y <- tuneR::readMP3(i)
  tuneR::writeWave(y, filename = paste0(substr(i, 1, nchar(i)-3), 'wav'))
}
...
```{r}
all.equal(survey, read.csv("./20200617_090000/Run1/Ex_20200617_090000_Run1_Settings.csv", stringsAsFactors = F))
```
...
or something else? Specifying a layout matrix gives us control over how we arrange the spectrograms, which naturally we want to imitate the spatial layout in the field (i.e. north up, east to the right).

In our case, the stations were numbered 1 to 9 from top left to bottom right.
...
The layoutMatrix function is very similar to the matrix function in base R, but results in a character matrix containing station names.
...
We can see nine spectrograms, and can also see that most of the sounds are visible (at the same time point) across multiple spectrograms. Adding some labels makes this clearer:

![](Sound0_Edit.png)

Five different individual birds sang during these seven seconds. Three of these can be safely ignored because they are singing from outside of the array and only perceptible at the edge. These are a House Wren in the top-right panel, a Clay-colored Sparrow in the bottom-right panel, and a Yellow Warbler in the bottom-right panel.

The first bird we will localize is the Red-eyed Vireo near microphone 8 (bottom-middle panel), which is visible on all nine microphones. This example is a good illustration of the advantage of manual detection methods for localization. If using automated methods, the six Red-eyed Vireo songs may have been detected up to 6*9 = 54 times across the nine channels. Thus, you would need some way of not only detecting individuals, but also accounting for the fact that the same sounds are detectable on many microphones. For a trained observer, that is quite trivial. 

The second bird we will localize is the Clay-colored Sparrow near the middle microphone. That Clay-colored Sparrow is a difficult case, because its song is quite faint. It can clearly be seen on microphone 5 (middle), but only faintly on (to my eye) microphones 2, 3, and 6. Clay-colored Sparrows sometimes sing quite faintly, and it can be surprising how their sound can be nearly inaudible just 40 meters away. We will try to localize it anyway.

## Filling out the Detections file.

Now that we have created the spectrogram (which would normally be many spectrograms for a longer recording session), we can fill out the detections file. To do so, simply open it in Excel, fill it in, and save it. Here's what the filled out detections file looks like:

```{r}
read.csv(system.file('data', 'Vignette_Detections_20200617_090000.csv', package = 'locaR'))
```

One row is entered per sound. Notice that for rows 6-8, I only entered a comment that I was ignoring a bird outside the array. That is a note for posteriority, so I know I have detected the bird, considered it, and decided to ignore it.
...
Both species were labeled with Individual 1, since they were the first individual of that species detected. Subsequent individuals of the same species would be labeled 2, 3, etc. to keep track of who is who within a survey.
