...
localizeDetection = function(st, index, tempC=15, plot=TRUE, InitData=NULL, 
  ...
  SearchMap = makeSearchMap(st, NodeInfo)
...
makeSearchMap = function(st, NodeInfo) {
  
  #Offset X, Y and Z coordinates to UTM grid.
  posdf = as.data.frame(NodeInfo$Pos)
  
  XLim1=min(posdf$Easting)-st$margin
  XLim2=max(posdf$Easting)+st$margin
  YLim1=min(posdf$Northing)-st$margin
  YLim2=max(posdf$Northing)+st$margin
  ZLim1=st$zMin+min(posdf$Elevation) #Z is offset by the minimum at the low end, and maximum at the high end.
  ZLim2=st$zMax+max(posdf$Elevation)
  
  XDen = YDen = ZDen = st$resolution
  
  XAxis  = seq(XLim1,XLim2, XDen)
  YAxis  = seq(YLim1,YLim2, YDen)
  ZAxis  = seq(ZLim1,ZLim2, ZDen)
  ...
  SearchMap = list(XMap = XMap, YMap = YMap, ZMap = ZMap, XDen = XDen, YDen = YDen, ZDen = ZDen,
