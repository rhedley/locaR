...
MSRP_Init = function(NodeInfo, SearchMap, Para, LevelFlag) {
  ...
  if (LevelFlag <2) {
    ...
    for(k in 1:kk) {
      ...
      d = 0.5*den*colMins(1/abs(rbind(cos(phi)*sin(theta),sin(phi)*sin(theta),cos(theta))))
      ...
      dT1 = colMaxs(rbind(ones,dT1)) #Use pmin and pmax? Or just write over values<1
      dT1 = colMins(rbind(onesML,dT1)) #Is this not just recursively writing over values?
      dT2 = colMaxs(rbind(ones,dT2)) #Use pmin and pmax? Or just write over values<1
      dT2 = colMins(rbind(onesML,dT2))
    ...
    print(paste('Created InitData in', round((proc.time()-xyz)[3],1), 'seconds'))
  }
