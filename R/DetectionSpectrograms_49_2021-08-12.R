...
#layoutMatrix creates a matrix of station names, which correspond to the layout of stations in space.
#This is passed to the omniSpectro function for the purposes of generating spectrograms that align
#with the spatial orientation of stations.
#The four user-specified arguments indicate where the lowest-numbered station occurs (e.g. topleft means
#the lowest numbered station is in the northwest).
#byrow means the numbers increase along rows (either left to right or right to left)
#and nrow and ncol indicate how many rows and columns of microphones there are (assuming the array
#has a rectangular shape).
#Future iterations will attempt to use coordinate locations to automate the layout, to avoid
#the user-specified arguments, as well as to allow non-rectangular shapes.

#Should change this to include an "auto" function.
layoutMatrix = function(st, start=c('topleft', 'topright', 'bottomleft', 'bottomright'),
                        byrow=TRUE, nrow, ncol) {

  stations = data.frame(station=sort(st$files$Station), row=NA, col=NA, stringsAsFactors=F)
  ...
  if(!byrow & start %in% c('topleft', 'bottomright')) {m = t(m)}
  if(!byrow & start %in% c('bottomleft', 'topright')) {
    m = m[nrow:1,]
    m = m[,ncol:1]
    m = t(m)
  }
...
#lm = layoutMatrix(st, start = 'topleft', byrow=F, nrow=7, ncol=7)
